    ...
    projects = props.blogs.map((b)=>(
      <div className="container col-sm-12 col-md-6 mt-4 mb-4" key={b._id}>
        <div>
          ...
          <img src={b.author.authorURL} className="ml-3"style={{borderRadius:'50%',width:'80px',position:'relative',bottom:'40px',float:'left'}} />
          <span className="float-left ml-2">By :<span className="text-primary">{b.author.username}</span></span>
          <span className="float-right">{b.created_date}</span>
        </div>
        <h2 className="text-left float-left"><a href="/">{b.title}</a></h2>
        <p className="text-left float-left" style={{height:'30vh',overflow:'hidden'}}>{b.content}</p>
